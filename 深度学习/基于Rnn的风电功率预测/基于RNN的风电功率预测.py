...
class PowerData(Dataset):
    def __init__(self,csv_path,sequnse_len):
        self.sequnse_len = sequnse_len
    ...
    def __len__(self):
        return  len(self.data) - self.sequnse_len
    def __getitem__(self,idx):
        start_idx = idx
        end_idx = idx+ self.sequnse_len
...
sequnse_len = 6
power_data = PowerData("./A01.csv", sequnse_len)
train_ratio = 0.8
val_ratio = 0.1
test_ratio = 0.1
...
for epoch in range(1,epochs+1):
    ...
    for batch_feature,batch_target in train_dataloder:
        ...
        loss = crition(y_hat,batch_target.view(-1))
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
with torch.no_grad():
    for batch_feature, batch_target in test_dataloder:
        ...
        batch_feature, batch_target = batch_feature.to(device), batch_target.to(device)
        y_hat = model(batch_feature)
        predict_list.append(y_hat.item())
